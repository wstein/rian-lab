...
Total **853/853** parity records across Lexer/TypeStr/Pratt/Core/Prim/Decl/Range/PatternLower/
...
(`Beam`/`JS`/`JVM`/`Lower`) — the value backend — plus, in `Check`, only `annotate` (needs a per-node
Core type field); the `check_program` gate is now complete (all 11 `check_func` checks). The parity-record count measures
