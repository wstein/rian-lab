...
| `Rian.Check`          | 2724 | 🟡 **stages 1-3a ported** — `unify`/`join` + `infer` over the expression core, `if`/`lambda`/`block` (branch-join + value unions + arrow types). `infer` case/calls/generics, `annotate`, error-sets, the program gates pending. **reframe, not lift**. |
...
Total **643/643** parity records across
