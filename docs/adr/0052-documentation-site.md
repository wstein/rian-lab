...
**Amended 2026-06-22 (ADR-0090):** superseding the above — the live engine is the **PureScript compiler compiled to JS by stock `purs`** (its native backend; ADR-0084 makes the compiler PureScript). The *whole real compiler* runs client-side as native JS — no WASM, no `v1==v2` gate. Self-host → JS is **demoted to a language milestone** (a later cross-check), and the `Rian.Eval` interpreter reverts to **oracle-only** (its WASM-engine role is dropped). Wherever this ADR says "self-host → JS" or "interpreter → WASM" *as the engine*, read "PureScript → JS" (ADR-0090).
