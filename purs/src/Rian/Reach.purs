...
scanFunc :: Array String -> Func -> Acc
scanFunc modnames f = foldl (scanClause modnames) { blockers: sigBlockers, callees: [] } f.clauses
  where
  ...
  sigBlockers = ref <> int <> width <> anyB
...
addBlocker b acc = acc { blockers = acc.blockers <> [ b ] }
